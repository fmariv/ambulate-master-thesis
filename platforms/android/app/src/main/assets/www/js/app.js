...
function registry() {
    let mail_input = $("#mail").val();
    let psw_input = $("#psw").val();
    alert(mail_input)
    alert(psw_input)
    ...
    if (!mailFail && !pswFail) {
        $.ajax({
            ...
            error: function(response) {
                document.getElementById('error-text').innerHTML = "No s'ha pogut registrar";
            }
        });
    } else if (!mailFail && pswFail) {
        document.getElementById('error-text').innerHTML = pswFail;
    } else if (mailFail && !pswFail) {
        document.getElementById('error-text').innerHTML = mailFail;
    } else if (mailFail && pswFail) {
        document.getElementById('error-text').innerHTML = mailFail + " i " + pswFail.toLowerCase();
    } 
}
